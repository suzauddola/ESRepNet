...
class conv5656(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=(1, 3, 5, 7), s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Conv, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.conv1x1 = nn.Conv2d(c1, c2, 1, 1, 0, groups=1, bias=True)
        self.conv3x3 = nn.Conv2d(c1, c2, 3, 1, 0, groups=1, bias=True)
        self.m = nn.ModuleList([nn.Conv2d(c1, c2, x, 1, 0, groups=1, bias=True) for x in k])
...
class mySPP(nn.Module):
  ...
  def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(1,3,5,7), l=(3,3), q=(3,5,9,13)):
      ...
      self.cv5 = Conv(4 * c_, c_, 3, 1)
    #   self.four_conv = nn.ModuleList([nn.Conv2d(c_, c_, kernel_size=x, stride=1) for x in k])
    #   self.conv_33 = nn.ModuleList([nn.Conv2d(c_, c_, kernel_size=x, stride=1) for x in l])
    #   self.conv_53 = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in q])
      self.cv5 = Conv(4 * c_, c_, 3, 1)
      self.cv6 = Conv(c_, c_, 3, 1)
      self.cv7 = Conv(2 * c_, c2, 1, 1)
      self.cv8 = Conv(c1, c_, 1, 1)
  ...
  def forward(self, x):
    ...
    #   t2 = self.cv3(self.cv2([k(t1) for k in self.four_conv]))
    #   t31 = [l(t2) for l in self.conv_33]
    #   t32 = [q(t2) for q in self.conv_53]
    #   t4 = self.cv4(torch.cat([t31+t32],1))

    #   t31_tensor = torch.stack(t31)
    #   t32_tensor = torch.stack(t32)
    #   print("t31_tensor ;", t31_tensor)
    #   print("t32_tensor ;", t32_tensor)

    #   t4 = self.cv4(torch.cat([t31, t32], 1))
    #   out = self.cv6(self.cv5(torch.cat(x+t4,1)))

    #   t6 = self.cv8(x)
    #   out = self.cv7(torch.cat((t5, t6), dim=1))
...
class mcff(nn.Module):
    ...
    def forward(self, x):
      ...
      cv7_7 = self.cv77(t1)
      out = self.cv2(torch.cat((cv1_1, cv3_3, cv5_5, cv7_7), dim=1))
      return self.cv3(torch.cat((x, out), dim=1))